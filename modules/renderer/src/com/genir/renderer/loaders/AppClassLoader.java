...
public class AppClassLoader extends ClassLoader {
      ...
      List.of(
          new ClassConstantTransformer(
              Arrays.asList(
                  ...
                  // Append FR version to title screen string.
                  ClassConstantTransformer.newTransform(
                      "Starsector 0.98a-RC8", "Starsector 0.98a-RC8 FR7.2"))),
          new ClassConstantTransformer(
              List.of(
  ...
  private HookRegistry initHooks() {
    ...
    // TextureLoader: wrap o00000 (loadTexture) with FR dispatch
    reg.register(
    ...
    // ResourceLoaderState: caller-scanning resolution (TODO: fragile, anchored on stable
    // ö00000 method name which could change on game update — revisit with interceptAndSkip
    // primitive once a more robust resolution strategy is identified)
  ...
  private Resolver buildResolver() {
    ...
    // TODO: Caller-scanning resolution for ResourceLoaderState hooks.
    // Discovers platform-varying SpecStore/ScriptStore method names by scanning
    // ResourceLoaderState.init(Map)V call sequence. Anchored on stable ö00000 name.
    // Fragile if game update changes call order — revisit if broken.
    ...
    String[] rlsNames = scanResourceLoaderInit();
    r.registerMethod("SpecStore", "init", rlsNames[0]);
    r.registerMethod("ScriptStore", "runScripts", rlsNames[1]);

    return r;
  }

  /**
   * TODO: Workaround — scans ResourceLoaderState.init(Map)V bytecode to discover platform-varying
   * method names by call-sequence position. Returns [specStoreInit, runScripts].
   */
  private String[] scanResourceLoaderInit() {
    byte[] bytes;
    try {
      bytes = loadFromGameJars("com/fs/starfarer/loading/ResourceLoaderState");
    } catch (Exception e) {
      throw new RuntimeException("Failed to load ResourceLoaderState from game jars", e);
    }
            ...
            new ClassVisitor(Opcodes.ASM9) {
              ...
              public MethodVisitor visitMethod(
                ...
                return new MethodVisitor(Opcodes.ASM9) {
                  String prevScriptStoreVoid = null;
                  ...
                  public void visitMethodInsn(
                    ...
                    if (owner.equals("com/fs/starfarer/loading/scripts/ScriptStore")
                        && d.equals("()V")) {
                      if (n.equals("\u00f600000")) {
                        result[1] = prevScriptStoreVoid;
                      }
                      prevScriptStoreVoid = n;
                    }
    ...
    if (result[0] == null || result[1] == null) {
      throw new RuntimeException(
          "ResourceLoaderState caller scan failed: specStoreInit="
              + result[0]
              + " runScripts="
              + result[1]);
    }
